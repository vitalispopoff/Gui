...
namespace Graph_lib 
{																
	//v/#undef major											/// defense against ill-behaved Linux macros:
	//v/#undef minor
	...
	struct Color 
	{
		enum class Color_type									//v/ made enum to enum class
		{
			red			= FL_RED, 
			blue		= FL_BLUE, 
			green		= FL_GREEN,
	...
	struct Line_style 
	{
			enum class Line_style_type  //v/ made enum to enum class
			{
				solid=FL_SOLID,				/// -------
				dash=FL_DASH,				/// - - - -
				dot=FL_DOT,					/// ....... 
	...
	{	
		...
		public:													
			...
			//{
			//	if (a.attached)
			//		error("attempt to copy attached shape");
			//}
	...
	{
		Fill() 
			: no_fill(true), fcolor(0) 
		{}
		Fill(Color c) 
			: no_fill(false), fcolor(c) 
		{}
	...
	struct Rectangle : Shape 
	{
		...
		//Color 
		//	fcolor;												/// fill color; 0 means "no fill"
	};
	...
	private:
		...
		Font 
			fnt	{fl_font()};
		int 
			fnt_sz {(fl_size() > 14) ? fl_size() : 14};			/// at least 14 point
	};

	struct Axis : Shape 
	{		
		enum class Orientation									/// representation left public
		{ 
			x, 
			y, 
			z 
	...
	struct Ellipse : Shape 
	{
		...
		{ 
			return
				{point(0).x + w, point(0).y + h }; 
		}
		Point	focus1		() const 
		{ 
			return
				{center().x + int(sqrt(double(w * w - h * h))), center().y }; 
		}
		Point	focus2		() const	
		{ 
			return
	...
	//		: Text(Point(xy.x - dw, xy.y + dh), string(1,c)) 
	//	{}
	//};
	...
	struct Image : Shape 
	{
		...
		private:
			int													/// define "masking box" within image relative to position (cx,cy)
				w,
				h,
				cx,
				cy;
