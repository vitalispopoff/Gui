...
template< class T> struct Vector : public std::vector<T>		/// trivially range-checked vector (no iterator checking):
{
	using size_type = typename std::vector<T>::size_type;

	#ifdef _MSC_VER												/// microsoft doesn't yet support C++11 inheriting constructors
	...
	#define vector Vector										/// disgusting macro hack to get a range checked vector:


	struct String : std::string									/// trivially range-checked string (no iterator checking)
	{
...
namespace std													/// of namespace std
{
...
inline void error(const string & s)								/// error() simply disguises throws:
{
